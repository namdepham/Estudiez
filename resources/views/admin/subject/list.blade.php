                            ...
                            <table class="table table-borderless table-data3">
                                ...
                                @foreach($data as $key => $row)
                                    <tr  class="text-center">
                                        ...
                                        <td>{{$row->name}}</td>
                                        <td>
                                            @foreach($dataResource as $line)
                                            {{$line->type}},
                                            @endforeach
                                        </td>
                                        <td>
                                            <a href="{{ route('subject.edit',$row->id) }}" class="btn btn-outline-primary"><i class="fa fa-edit"></i> Edit</a>
                                        </td>
                                        <td>
                                            <form action ="{{ route('subject.destroy', $row) }}" method="POST">
                                                @method('delete')
                                                @csrf
